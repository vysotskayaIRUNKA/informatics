...
public:
  ...
  void do_code(int code_number) {
    ...
    int success_chance =
        chance_of_luck +
        (code_completed * 5); // Чем больше сделано лаб, тем выше шанс
  ...
  void submit_project() {
    ...
    if (chance_of_luck > bugs * 15) {
      ...
      std::cout
          << "Багов оказалось слишком много... Проект отправлен на доработку."
          << std::endl;
      chance_of_luck -= 15;
    }
  }
...
first_one.print();
  */
  ...
  lin.exam();
    ...
    std::cout << "\n--- Пытаемся сдать проект до завершения всех кодов ---" << std::endl;
    ...
    std::cout << "\n--- Используем навыки из Informatics (делаем лабы) ---" << std::endl;
    ...
    std::cout << "\n--- Имитируем поломку инструментов (вызовем печать несколько раз) ---" << std::endl;
    for (int i = 0; i < 3; i++) {
