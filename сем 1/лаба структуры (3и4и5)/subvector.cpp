...
int main()
{
	...
	finish = get_time();
	for (int i = 0; i < n; i++) //!!! This is a hack to bamboozle the O3
	                            //! optimization.
	...
	cout << "003 Random pop/push equal amount: \t" << finish - start << "\t\t"
	     << sum_for_O3 << endl;
	total += finish - start;
	...
	clear(&sv);
	shrink_to_fit(&sv);
	cout << "004 Random pop/push more push: \t\t" << finish - start << "\t\t"
	     << sum_for_O3 << endl;
	total += finish - start;
	...
	finish = get_time();
	cout << "005 Random pop/push much more push: \t" << finish - start << "\t\t"
	     << sum_for_O3 << endl;
	total += finish - start;
	//----------- End of tests
	destructor(&sv);
